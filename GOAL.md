...
- Keep every executable fixture workflow, local action, Rust example, test
  ...
  depend on the retired external workflow provider; generated workflows come
  from `velnor-workflow`.
...
1. Capability coverage is closed: exactly 29 admitted action repositories and
   one admitted reusable workflow have one coverage row each.
2. Positive workflow dispatch inputs default to `both` and automatic positive
   triggers execute both lanes.
3. No workflow executes the retired external provider; all workflow templates
   and outputs are generated by `velnor-workflow`.
